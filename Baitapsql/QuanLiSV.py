...
# # Kết nối tới db
#conn = sqlite3.connect('QuanLiSV.db')
#c = conn.cursor()
...
def chinh_sua():
    ...
    # Kiểm tra nếu ô delete_box trống (chưa chọn ID)
    if delete_box.get() == "":
        messagebox.showerror("Lỗi", "Vui lòng chọn một bản ghi để chỉnh sửa.")
        return  # Dừng thực hiện nếu không có ID

    # Gán giá trị cho record_id từ delete_box
    record_id = delete_box.get()

    # Nếu ID hợp lệ, tiếp tục thực hiện
    editor = Tk()
    ...
    # Lấy dữ liệu của bản ghi với ID tương ứng
    ...
    # Nút để lưu bản ghi đã chỉnh sửa
